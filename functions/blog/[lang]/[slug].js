...
(function(){
  ...
  fetch(ANA,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
    site_id:'${siteId}',post_id:${postId},post_slug:'${postSlug}',lang:'${lang}',
    event:'pageview',referrer:REF,device:DEV,session_id:SID
  })}).catch(function(){});
  ...
  function onScroll(){
    var el=document.querySelector('.blog-post-body,.post-body');
    if(!el)return;
    var r=el.getBoundingClientRect();
    var pct=Math.min(100,Math.round((window.innerHeight-r.top)/(r.height||1)*100));
  ...
  // Zeit auf Seite beim Verlassen
  function sendTime(){
    if(sent)return;sent=true;
    var sec=Math.round((Date.now()-START)/1000);
    if(sec<2)return;
    navigator.sendBeacon(ANA,JSON.stringify({
      site_id:'${siteId}',post_id:${postId},post_slug:'${postSlug}',lang:'${lang}',
...
function feedbackWidget(postId, postSlug, lang, siteId) {
  ...
  const l = labels;
  const q = l.question[lang] || l.question.en;
  const y = l.yes[lang]      || l.yes.en;
  const n = l.no[lang]       || l.no.en;
  const t = l.thanks[lang]   || l.thanks.en;
  const key = `fb_${postId}_${lang}`;
  return `
<div id="feedback-widget" style="margin-top:2.5rem;padding:1.25rem 1.5rem;border:1px solid rgba(255,255,255,.08);border-radius:12px;background:rgba(255,255,255,.02);display:none;text-align:center">
  <p id="fb-question" style="font-size:.88rem;color:var(--text-secondary);margin:0 0 .9rem;letter-spacing:.01em">${q}</p>
